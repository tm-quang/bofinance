...
export const WelcomeModal = ({ isOpen, onClose }: WelcomeModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      {/* Backdrop with blur */}
      <div
        className={`absolute inset-0 bg-gradient-to-br from-slate-900/70 via-slate-800/70 to-slate-900/70 backdrop-blur-md transition-opacity duration-300 ${
      ...
      {/* Modal */}
      <div
        className={`relative w-full max-w-lg transform overflow-hidden rounded-3xl bg-white shadow-[0_25px_80px_rgba(0,0,0,0.5)] ring-1 ring-slate-200/50 transition-all duration-500 ${
          isVisible ? 'scale-100 opacity-100' : 'scale-95 opacity-0'
        }`}
      >
        {/* Decorative gradient header */}
        <div className="absolute top-0 left-0 right-0 h-32 bg-gradient-to-br from-sky-500 via-blue-500 to-indigo-600 opacity-10" />
        ...
        <div className="relative flex flex-col items-center px-6 pt-8 pb-6">
          {/* Logo with animation */}
          <div className={`mb-4 flex items-center justify-center transition-all duration-500 ${
          ...
          {/* Welcome text */}
          <div className={`mb-6 text-center transition-all duration-500 delay-100 ${
          ...
          {/* App description */}
          <div className={`mb-6 w-full text-center transition-all duration-500 delay-200 ${
            isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'
          }`}>
          ...
          {/* Features grid */}
          <div className={`mb-6 grid w-full grid-cols-2 gap-3 transition-all duration-500 delay-300 ${
            isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'
          }`}>
            {APP_FEATURES.map((feature, index) => {
              ...
              return (
                <div
                  key={feature.title}
                  className={`group relative overflow-hidden rounded-2xl border border-slate-100 bg-gradient-to-br from-white to-slate-50 p-4 shadow-sm transition-all duration-300 hover:scale-[1.02] hover:shadow-md ${
                    featureIndex === index ? 'ring-2 ring-sky-400 ring-offset-2' : ''
                  }`}
                >
          ...
          <div className={`mt-4 w-full transition-all duration-500 delay-500 ${
            isVisible ? 'opacity-100' : 'opacity-0'
          }`}>
