/**
 * VEHICLE MANAGEMENT - SUPABASE SQL SCHEMA
 * =========================================
 * 
 * HƯỚNG DẪN SETUP DATABASE:
 * 
 * 1. Copy toàn bộ nội dung SQL từ constant SETUP_SQL bên dưới
 * 2. Mở Supabase Dashboard (https://app.supabase.com)
 * 3. Chọn project của bạn
 * 4. Vào SQL Editor (biểu tượng database ở sidebar trái)
 * 5. Nhấn "New query"
 * 6. Paste SQL vào editor
 * 7. Nhấn "Run" hoặc Ctrl/Cmd + Enter
 * 8. Chờ vài giây để schema được tạo
 * 9. Kiểm tra Table Editor để xem các bảng đã được tạo
 * 
 * Hệ thống sẽ tạo 5 bảng:
 * - vehicles (Thông tin xe)
 * - vehicle_trips (Hành trình)
 * - vehicle_fuel_logs (Nhiên liệu)
 * - vehicle_maintenance (Bảo dưỡng)
 * - vehicle_expenses (Chi phí khác)
 */
...
DROP POLICY IF EXISTS "Users can manage their own trips" ON vehicle_trips;
DROP POLICY IF EXISTS "Users can manage their own fuel logs" ON vehicle_fuel_logs;
DROP POLICY IF EXISTS "Users can manage their own maintenance" ON vehicle_maintenance;
DROP POLICY IF EXISTS "Users can manage their own expenses" ON vehicle_expenses;
...
DROP TRIGGER IF EXISTS update_vehicle_trips_updated_at ON vehicle_trips;
DROP TRIGGER IF EXISTS update_vehicle_fuel_logs_updated_at ON vehicle_fuel_logs;
...
console.log('📋 COPY SQL BELOW AND RUN IN SUPABASE SQL EDITOR:');
